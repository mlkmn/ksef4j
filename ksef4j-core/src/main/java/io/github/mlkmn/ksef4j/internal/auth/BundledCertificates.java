...
/**
 * Internal: loads a bundled X.509 certificate for an (environment, usage) pair from {@code
 * /keys/<env>-<usage>.pem} on the classpath. v0.1 bundles TEST, DEMO and PROD, both usages. Shared
 * by {@link ClasspathKeyResolver} and {@link HttpCertificateResolver}.
 */
final class BundledCertificates {
  ...
  static X509Certificate load(Environment environment, KeyUsage usage) {
    String resource =
        "/keys/" + environment.name().toLowerCase(Locale.ROOT) + "-" + suffix(usage) + ".pem";
    try (InputStream in = BundledCertificates.class.getResourceAsStream(resource)) {
      if (in == null) {
        throw new KsefAuthenticationException(
            "Bundled KSeF certificate missing from classpath: " + resource);
      }
      ...
      throw new KsefAuthenticationException(
          "Failed to parse bundled KSeF certificate: " + resource, e);
    }
  }
