 ...
 * /keys/<env>-upo-signing.pem}. TEST, DEMO and PROD are all bundled; loading throws {@link
 * UpoVerificationException} if a certificate is unexpectedly absent. Caches per environment.
 */
    ...
    try (InputStream in = UpoSigningCertificates.class.getResourceAsStream(resource)) {
      if (in == null) {
        throw new UpoVerificationException(
            "UPO signing certificate not bundled for "
                + environment
                + " (expected "
                + resource
                + ")");
      ...
      throw new UpoVerificationException(
          "Failed to parse bundled UPO signing certificate: " + resource, e);
    }
  }
}
