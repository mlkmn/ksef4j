...
/**
 * An immutable, validated invoice metadata query. Build via {@link #asSeller()} or {@link
 * #asBuyer()}, set a required date range, then {@link Builder#build()}.
 *
 * <p>Invalid queries (missing/inverted/over-long date range, out-of-bounds paging) throw {@link
 * IllegalArgumentException} at build time - they are caller errors, not remote failures.
 */
public final class InvoiceQuery {
  ...
  public String counterpartyNip() {
    return counterpartyNip;
  }
  ...
  public static final class Builder {
    ...
    /**
     * Convenience: filter by the KSeF invoicing (registration) date between {@code from} and {@code
     * to}.
     */
    ...
    public Builder counterpartyNip(String nip) {
      this.counterpartyNip = nip;
      return this;
    }
    ...
    public Builder ksefNumber(String ksefNumber) {
    ...
    public Builder pageOffset(int pageOffset) {
     ...
     *
     * @throws IllegalArgumentException if the date range is missing, inverted, longer than {@link
     *     #MAX_RANGE_MONTHS}, or paging is out of bounds
     */
    public InvoiceQuery build() {
      if (dateType == null || from == null || to == null) {
        throw new IllegalArgumentException(
            "A date range is required (call issuedBetween/receivedBetween/dateRange)");
      }
      if (from.isAfter(to)) {
      ...
      if (to.isAfter(from.plusMonths(MAX_RANGE_MONTHS).minusDays(1))) {
        throw new IllegalArgumentException(
            "Date range may span at most "
                + MAX_RANGE_MONTHS
                + " calendar months minus one day "
                + "(the end date is inclusive); got "
                + from
                + " to "
                + to);
      }
