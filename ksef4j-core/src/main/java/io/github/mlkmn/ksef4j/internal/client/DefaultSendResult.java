 ...
 * archives the completed record best-effort, and guards against use after {@link #close()}. Not
 * thread-safe; single-caller use within try-with-resources.
 */
final class DefaultSendResult implements SendResult {
  ...
  public Upo awaitUpo() {
    ...
    if (cachedUpo == null) {
      Upo upo =
          poller.pollUntilSettled(
              sessionReferenceNumber, invoiceReferenceNumber, accessToken, upoPollTimeout);
  ...
  private void verifyDocumentHash(Upo upo) {
    ...
    if (!expected.equals(declared)) {
      throw new UpoVerificationException(
          "UPO document hash mismatch for invoice "
