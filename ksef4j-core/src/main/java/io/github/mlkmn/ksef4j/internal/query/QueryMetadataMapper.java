...
public final class QueryMetadataMapper {
    ...
    public static InvoiceMetadataPage toPage(Responses.QueryMetadata wire, int pageOffset, int pageSize) {
        List<InvoiceMetadata> invoices = wire.invoices().stream().map(QueryMetadataMapper::toMetadata).toList();
        return new InvoiceMetadataPage(invoices, wire.hasMore(), wire.isTruncated(), pageOffset, pageSize);
    }
    ...
    private static Counterparty buyer(Responses.QueryMetadata.Buyer b) {
        ...
        String nip = (b.identifier() != null && "Nip".equals(b.identifier().type()))
                ? b.identifier().value() : null;
