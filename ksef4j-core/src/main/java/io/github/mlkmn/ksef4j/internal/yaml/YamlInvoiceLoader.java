...
/**
 * Private YAML loader for {@link Invoice}. Used by {@link Invoice#fromYaml(java.nio.file.Path)} and
 * its overloads. Configures a single {@link ObjectMapper} once at class-load time; the
 * configuration is intentionally not exposed - v0.1 locks the YAML schema and the parser knobs.
 *
 * <p>All failures (parse, IO, unknown key, missing required, type mismatch, unsupported VatRate)
 * surface as {@link InvoiceValidationException}, with the underlying Jackson or IO exception set as
 * the cause.
 */
