...
/**
 * Validates an {@link Invoice} against a curated list of FA(3)-derived
 * constraints. Pure function, stateless, thread-safe. Fails fast on the first
 * violation by throwing {@link InvoiceValidationException} with a message
 * naming the offending field and value.
 *
 * <p>Rules cite the FA(3) v1-0E simple type they derive from. Currency,
 * country, items-empty and address-presence checks live in
 * {@link InvoiceMapper} and {@code TKodWaluty.fromValue} / {@code
...
public final class InvoiceValidator {
    ...
    private static void checkCurrencyAndRate(String currency, BigDecimal exchangeRate) {
        ...
        if (exchangeRate == null || exchangeRate.signum() <= 0) {
            throw fail("Invoice in currency " + currency
                    + " requires a strictly positive exchange rate, got " + exchangeRate);
        }
    }
    ...
    private static void checkInvoiceNumber(String invoiceNumber) {
        ...
        if (invoiceNumber.length() > MAX_256) {
            throw fail("Invoice number must be at most " + MAX_256 + " characters, got " + invoiceNumber.length());
        }
    }
    ...
    private static void checkAddress(String label, Address address) {
        ...
        if (address.line1().length() > MAX_512) {
            throw fail(label + " line 1 must be at most " + MAX_512 + " characters, got " + address.line1().length());
        }
        if (address.line2() != null && address.line2().length() > MAX_512) {
            throw fail(label + " line 2 must be at most " + MAX_512 + " characters, got " + address.line2().length());
        }
    }
    ...
    private static void checkItem(int rowNumber, Item item) {
        ...
        if (item.description().length() > MAX_512) {
            throw fail(prefix + " description must be at most " + MAX_512 + " characters, got " + item.description().length());
        }
        ...
        if (item.unit() != null && item.unit().length() > MAX_256) {
            throw fail(prefix + " unit must be at most " + MAX_256 + " characters, got " + item.unit().length());
        }
        if (item.pkwiu() != null && item.pkwiu().length() > MAX_50) {
            throw fail(prefix + " PKWiU must be at most " + MAX_50 + " characters, got " + item.pkwiu().length());
        }
    }
