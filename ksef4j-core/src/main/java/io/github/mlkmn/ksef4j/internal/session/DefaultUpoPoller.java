...
public final class DefaultUpoPoller implements UpoPoller {
    ...
    @Override
    public Upo pollUntilSettled(String sessionReferenceNumber, String invoiceReferenceNumber,
                                String accessToken, Duration timeout) {
        Instant deadline = clock.instant().plus(timeout);
        int attempt = 1;

        // Phase 1: the per-invoice status is the authoritative accept/reject signal for a single
        // invoice (a session can report 200 "processed" with no UPO even when its invoice was rejected).
        while (true) {
            ...
            if (!isProcessing(code)) {
                throw new KsefBusinessException(String.valueOf(code), STATUS_SUCCESS,
        ...
        while (true) {
            Responses.SessionStatus status = transport.fetchSessionStatus(sessionReferenceNumber, accessToken);
            ...
            if (!isProcessing(code) && code != STATUS_SUCCESS) {
                throw new KsefBusinessException(String.valueOf(code), STATUS_SUCCESS,
                        "KSeF session failed during UPO generation (session status " + code + "): "
    ...
    private int backoffOrTimeout(Instant deadline, int attempt, Duration timeout, String sessionRef) {
        if (!clock.instant().isBefore(deadline)) {
            throw new UpoTimeoutException(timeout,
                    "UPO not ready within " + timeout + " for session " + sessionRef);
        }
        try {
    ...
    private Upo assemble(Responses.UpoPage page) {
        ...
        return new Upo(parsed.ksefReferenceNumber(), page.referenceNumber(), parsed.issuedAt(),
                parsed.documentHash(), parsed.invoiceNumber(), xml);
    }
}
