...
public final class Requests {
    ...
    public record QueryMetadata(
        ...
        public static QueryMetadata from(InvoiceQuery q) {
            String subject = switch (q.role()) {
                case SELLER -> "Subject1";
                case BUYER -> "Subject2";
            };
            String dateType = switch (q.dateType()) {
                case ISSUE -> "Issue";
                case INVOICING -> "Invoicing";
                case PERMANENT_STORAGE -> "PermanentStorage";
            };
            DateRange range = new DateRange(dateType,
            ...
            BuyerIdentifier buyerId = (q.role() == SubjectRole.SELLER && q.counterpartyNip() != null)
                    ? new BuyerIdentifier("Nip", q.counterpartyNip()) : null;
            ...
            return new QueryMetadata(subject, range, sellerNip, buyerId,
                    q.ksefNumber(), q.invoiceNumber(), currencies, types);
        }
    }
}
