 ...
 * to be used as a singleton (Spring bean, manual cache, etc.).
 */
public interface KsefClient {
    ...
    final class Builder {
         ...
         * default host -- e.g. a reverse proxy or API gateway that fronts the selected environment,
         * or an in-process simulator. The environment still selects the encryption certificate, so
         * this is not a way to reach a different KSeF instance. Credentials and encrypted invoices
         * flow over this URL; point it only at a trusted endpoint. Defaults to the environment host.
         */
        ...
        /** Archive sent invoices to a directory tree (convenience for a {@link FilesystemInvoiceArchive}). */
        public Builder archiveDirectory(Path directory) {
            this.archiveDirectory = directory;
            return this;
        }

        /** Archive sent invoices through a custom {@link InvoiceArchive}. Mutually exclusive with {@link #archiveDirectory(Path)}. */
         ...
         *
         * @throws IllegalStateException if {@code environment} was not set, if
         *         {@code tokenAuth(token, nip)} was not set (or token/nip is blank),
         *         or if both {@code archive(...)} and {@code archiveDirectory(...)} were set
         */
        public KsefClient build() {
            ...
            if (archive != null && archiveDirectory != null) {
                throw new IllegalStateException("Set either archive(...) or archiveDirectory(...), not both");
            }

            EnvironmentEndpoints endpoints = (baseUrl != null)
                    ? EnvironmentEndpoints.ofBaseUri(baseUrl)
                    : EnvironmentEndpoints.of(environment);
            HttpTransport transport = new JdkHttpTransport(endpoints, httpConnectTimeout, httpRequestTimeout);
            Clock clock = Clock.systemUTC();
            KeyResolver keyResolver = new HttpCertificateResolver(transport, clock);

            AuthSession auth = new DefaultAuthSession(
                    transport, keyResolver, environment, token, nip, clock,
                    AUTH_POLL_INTERVAL, AUTH_TIMEOUT);
            InteractiveSession session = new DefaultInteractiveSession(
                    transport, keyResolver, environment, new SecureRandom());
            UpoPoller poller = new DefaultUpoPoller(transport, new ExponentialBackoff(), clock);

            UpoSignatureCheck signatureCheck = verifyUpoSignature
                    ? xml -> new UpoSignatureVerifier().verify(xml, environment)
                    : null;
