 ...
 * per-usage X.509 certificate ({@code /keys/<env>-<usage>.pem}), cached. v0.1 bundles TEST,
 * DEMO and PROD. Thread-safe; shared across concurrent sends.
 */
public final class ClasspathKeyResolver implements KeyResolver {
    ...
    public PublicKey publicKey(Environment environment, KeyUsage usage) {
        return cache.computeIfAbsent(new CacheKey(environment, usage),
