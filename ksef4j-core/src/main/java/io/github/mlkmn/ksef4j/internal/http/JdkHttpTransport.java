...
public final class JdkHttpTransport implements HttpTransport {
    ...
    public JdkHttpTransport(EnvironmentEndpoints endpoints, Duration connectTimeout, Duration requestTimeout) {
    ...
    public Responses.Challenge fetchChallenge() {
        return sendForJson(request(endpoints.challenge(), null).POST(BodyPublishers.noBody()).build(),
    ...
    public Responses.AuthSubmit submitKsefTokenAuth(String challenge, String nip, String encryptedTokenBase64) {
        Requests.KsefTokenAuth body = new Requests.KsefTokenAuth(
    ...
    public Responses.AuthStatus pollAuthStatus(String referenceNumber, String authToken) {
        return sendForJson(request(endpoints.authStatus(referenceNumber), authToken).GET().build(),
                Responses.AuthStatus.class);
    }

    @Override
    public Responses.TokenPair redeemTokens(String authToken) {
        return sendForJson(request(endpoints.tokenRedeem(), authToken).POST(BodyPublishers.noBody()).build(),
                Responses.TokenPair.class);
    }

    @Override
    public Responses.AccessToken refreshToken(String refreshToken) {
        return sendForJson(request(endpoints.tokenRefresh(), refreshToken).POST(BodyPublishers.noBody()).build(),
                Responses.AccessToken.class);
    }

    @Override
    public Responses.OpenSession openSession(Requests.OpenSession request, String accessToken) {
        return sendForJson(jsonPost(endpoints.openSession(), request, accessToken), Responses.OpenSession.class);
    }

    @Override
    public Responses.SendInvoice sendInvoice(String sessionRef, Requests.SendInvoice request, String accessToken) {
        return sendForJson(jsonPost(endpoints.sendInvoice(sessionRef), request, accessToken),
                Responses.SendInvoice.class);
    }

    @Override
    public Responses.SessionStatus fetchSessionStatus(String sessionRef, String accessToken) {
        return sendForJson(request(endpoints.sessionStatus(sessionRef), accessToken).GET().build(),
                Responses.SessionStatus.class);
    }

    @Override
    public Responses.InvoiceStatus fetchInvoiceStatus(String sessionReferenceNumber, String invoiceReferenceNumber,
                                                      String accessToken) {
        return sendForJson(
                request(endpoints.invoiceStatus(sessionReferenceNumber, invoiceReferenceNumber), accessToken)
                        .GET().build(),
    ...
    public void closeSession(String sessionRef, String accessToken) {
        ...
        sendExpectingSuccess(request(endpoints.closeSession(sessionRef), accessToken)
    ...
    public List<Responses.CertificateInfo> fetchCertificates() {
        Responses.CertificateInfo[] certs = sendForJson(
    ...
    KsefException errorFor(HttpResponse<byte[]> response, URI uri) {
        ...
        if (status >= 400 && status < 500) {
            String retry = response.headers().firstValue("Retry-After")
                    .map(r -> "; Retry-After=" + r).orElse("");
            String message = "KSeF rejected the request (HTTP " + status + "): " + detail.summary() + retry;
            return new KsefBusinessException(detail.exceptionCode(), status, message);
        }
        return new KsefTransportException("KSeF transport error (HTTP " + status + "): " + detail.summary());
    }
    ...
    private ErrorDetail extractErrorDetail(byte[] body) {
        try {
            ...
            if (!description.isEmpty() || !code.isEmpty()) {
                String summary = "code=" + code + " " + description
