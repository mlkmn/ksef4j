...
/**
 * Internal: resolves KSeF v2 base URLs per environment and builds the full
 * request URI for each endpoint. {@link #ofBaseUri(URI)} is the testability
 * seam (point at an in-process server).
 */
public final class EnvironmentEndpoints {
    ...
    public static EnvironmentEndpoints of(Environment environment) {
        return new EnvironmentEndpoints(switch (environment) {
    ...
    public URI challenge() { return URI.create(base + "/auth/challenge"); }

    public URI ksefTokenAuth() { return URI.create(base + "/auth/ksef-token"); }

    public URI authStatus(String ref) { return URI.create(base + "/auth/" + ref); }

    public URI tokenRedeem() { return URI.create(base + "/auth/token/redeem"); }

    public URI tokenRefresh() { return URI.create(base + "/auth/token/refresh"); }

    public URI openSession() { return URI.create(base + "/sessions/online"); }

    public URI sendInvoice(String sessionRef) { return URI.create(base + "/sessions/online/" + sessionRef + "/invoices"); }

    public URI sessionStatus(String sessionRef) { return URI.create(base + "/sessions/" + sessionRef); }
