...
/**
 * Internal seam: low-level HTTP calls to KSeF v2. One request / one response
 * per method; no polling, backoff, or orchestration (that is Wave B). Returns
 * wire DTOs from {@link Responses}.
 */
public interface HttpTransport {

    Responses.Challenge fetchChallenge();

    Responses.AuthSubmit submitKsefTokenAuth(String challenge, String nip, String encryptedTokenBase64);
    ...
    Responses.SendInvoice sendInvoice(String sessionRef, Requests.SendInvoice request, String accessToken);
    ...
    Responses.InvoiceStatus fetchInvoiceStatus(String sessionReferenceNumber, String invoiceReferenceNumber,
                                               String accessToken);
