...
/**
 * Internal: provides a valid KSeF access token, caching it and refreshing (refresh-token then full
 * re-auth) as needed. Thread-safe; concurrent callers share one in-flight refresh.
 */
public final class DefaultAuthSession implements AuthSession {
  ...
  private void handshake() {
    ...
    } catch (KsefBusinessException e) {
      throw new KsefAuthenticationException(
          "KSeF authentication handshake failed: " + e.getMessage(), e);
    }
  }
  ...
  private static CachedToken cached(Responses.Token token) {
    try {
      ...
      throw new KsefAuthenticationException(
          "Unparseable token validUntil: " + token.validUntil(), e);
    }
  }
}
