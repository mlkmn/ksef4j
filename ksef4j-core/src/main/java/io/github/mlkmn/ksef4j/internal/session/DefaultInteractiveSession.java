...
/**
 * Internal: opens a KSeF interactive session, sends one encrypted FA(3) invoice, and closes the
 * session (close is the UPO prerequisite). Stateless; safe to share.
 */
public final class DefaultInteractiveSession implements InteractiveSession {
  ...
  public DefaultInteractiveSession(
  ...
  public SendReceipt send(byte[] fa3Xml, String accessToken) {
    ...
    byte[] wrappedKey =
        key.wrapKey(keyResolver.publicKey(environment, KeyUsage.SYMMETRIC_KEY_ENCRYPTION));
    Requests.OpenSession openRequest =
        Requests.OpenSession.from(wrappedKey, key.iv(), FA3_FORM_CODE);
    ...
    try {
      ...
      invoiceRef =
          transport
              .sendInvoice(sessionRef, Requests.SendInvoice.from(encrypted), accessToken)
              .referenceNumber();
