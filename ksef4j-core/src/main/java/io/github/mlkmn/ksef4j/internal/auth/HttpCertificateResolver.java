...
/**
 * Default {@link KeyResolver}: reads bundled certificates first (no network) and only fetches
 * fresh certificates from KSeF to refresh a cert that is missing or within {@code refreshSkew}
 * of expiry. Selects by usage, preferring the latest {@code validFrom}; caches per (env, usage).
 * On fetch failure it falls back to the bundled cert as a last resort. Thread-safe.
 */
public final class HttpCertificateResolver implements KeyResolver {
    ...
    public PublicKey publicKey(Environment environment, KeyUsage usage) {
        ...
        if (bundled != null && fresh(bundled.getNotAfter().toInstant())) {
            cache.put(cacheKey, new CachedCert(bundled.getPublicKey(), bundled.getNotAfter().toInstant()));
            return bundled.getPublicKey();
        }

        LOG.log(Level.WARNING, "Bundled KSeF {0} {1} certificate is missing or near/past expiry; "
                + "fetching a fresh certificate from KSeF. Update the bundled certificate.", environment, usage);
        try {
            X509Certificate fetched = selectFetched(environment, usage);
            cache.put(cacheKey, new CachedCert(fetched.getPublicKey(), fetched.getNotAfter().toInstant()));
            return fetched.getPublicKey();
        } catch (RuntimeException fetchFailure) {
            if (bundled != null) {
                LOG.log(Level.WARNING, "KSeF certificate fetch failed for {0} {1}; falling back to an "
                        + "expired/near-expiry bundled certificate. Operations may fail until refreshed.",
                        environment, usage);
                return bundled.getPublicKey();
            }
            throw new KsefAuthenticationException(
                    "No KSeF certificate available for " + environment + " " + usage + " (no bundle, fetch failed)",
                    fetchFailure);
        }
    }
