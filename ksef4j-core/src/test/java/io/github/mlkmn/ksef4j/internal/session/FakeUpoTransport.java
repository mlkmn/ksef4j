...
final class FakeUpoTransport implements HttpTransport {
  ...
  final Deque<Responses.SessionStatus> statuses =
      new ArrayDeque<>(); // consumed in order; last repeats
  byte[] upoXml;
  final AtomicInteger sessionStatusCount = new AtomicInteger();
  volatile URI lastUpoUrl;
  final Deque<Responses.InvoiceStatus> invoiceStatuses =
      new ArrayDeque<>(); // consumed in order; last repeats
  ...
  static Responses.SessionStatus ready(String pageRef, String downloadUrl) {
    return status(
        200, new Responses.Upo(List.of(new Responses.UpoPage(pageRef, downloadUrl, null))));
  }
