...
/**
 * Live read smoke test against the KSeF {@code test} environment: queries invoice
 * metadata for the configured NIP. Opt-in; run with
 * {@code ./gradlew :ksef4j-core:smokeTest --tests "*LiveQuerySmokeTest"} after
 * exporting {@code KSEF_TOKEN} and {@code COMPANY_NIP}. Self-skips when absent.
 *
 * <p>Read-only: it queries metadata the NIP is a party to (sales invoices the smoke
 * send tests have created over time) and never sends anything.
 */
@Tag("smoke")
class LiveQuerySmokeTest {
    ...
    void queries_seller_invoice_metadata_from_test_environment() {
        ...
        InvoiceQuery query = InvoiceQuery.asSeller()
                ...
                .pageSize(50)
                .build();
        System.out.println("[smoke-query] querying invoices issued as seller " + nip
                + " between " + from + " and " + to + " on KSeF TEST");
        ...
        System.out.println("[smoke-query] page returned: " + page.invoices().size()
                + " invoice(s), hasMore=" + page.hasMore() + ", nextOffset=" + page.nextOffset());

        for (InvoiceMetadata m : page.invoices()) {
            System.out.println("[smoke-query]   " + m.ksefNumber()
                    + " | no=" + m.invoiceNumber()
                    + " | issued=" + m.issueDate()
                    + " | seller=" + (m.seller() == null ? null : m.seller().nip())
                    + " | buyer=" + (m.buyer() == null ? null : m.buyer().nip())
                    + " | gross=" + m.grossAmount()
                    + " | schema=" + m.schema()
                    + " | type=" + m.invoiceType());
        }
        ...
        if (!page.invoices().isEmpty()) {
            ...
            System.out.println("[smoke-query] confirmed: real KSeF metadata parsed (ksefNumber + seller populated)");
        } else {
            System.out.println("[smoke-query] NOTE: zero invoices in the window - the request shape and response "
        ...
        long streamed;
        try (Stream<InvoiceMetadata> stream = client.streamInvoices(
                InvoiceQuery.asSeller().issuedBetween(from, to).pageSize(10).build())) {
            streamed = stream.count();
        }
        System.out.println("[smoke-query] streamInvoices walked " + streamed + " invoice(s) across pages");
        assertThat(streamed).isGreaterThanOrEqualTo(page.invoices().size());

        System.out.println("[smoke-query] confirmed: queryInvoices + streamInvoices succeeded against KSeF TEST");
    }
}
