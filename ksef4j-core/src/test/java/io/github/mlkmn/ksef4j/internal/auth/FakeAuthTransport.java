...
/**
 * Scriptable fake HttpTransport for AuthSession tests. Auth methods only; session methods are
 * unused.
 */
final class FakeAuthTransport implements HttpTransport {
  ...
  final Deque<Integer> statusCodes =
      new ArrayDeque<>(); // consumed in order; last value repeats; empty -> 200
  Responses.TokenPair tokenPair;
  Responses.AccessToken refreshResponse;
  KsefException refreshError;
  KsefException challengeError;
  KsefException submitError;
  ...
  public Responses.AuthSubmit submitKsefTokenAuth(
      String challengeValue, String nip, String encryptedTokenBase64) {
  ...
  public Responses.InvoiceStatus fetchInvoiceStatus(
      String sessionRef, String invoiceRef, String accessToken) {
