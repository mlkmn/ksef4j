...
class ExponentialBackoffTest {
  ...
  void schedule_doubles_and_caps_without_jitter() {
    ExponentialBackoff b =
        new ExponentialBackoff(Duration.ofMillis(500), Duration.ofSeconds(4), () -> 0.5);
  ...
  void jitter_stays_within_band() {
    ExponentialBackoff low =
        new ExponentialBackoff(Duration.ofMillis(500), Duration.ofSeconds(4), () -> 0.0);
    ExponentialBackoff high =
        new ExponentialBackoff(Duration.ofMillis(500), Duration.ofSeconds(4), () -> 0.999999);
    assertThat(low.delayBeforeAttempt(1)).isEqualTo(Duration.ofMillis(400)); // 0.8 * 500ms
    assertThat(high.delayBeforeAttempt(1))
        .isBetween(Duration.ofMillis(599), Duration.ofMillis(600)); // ~1.2 * 500ms
  }
}
