...
class UpoXmlSignatureTest {
    ...
    void document_without_signature_fails() throws Exception {
        byte[] unsigned = ("<Potwierdzenie xmlns=\"" + NS + "\"><Dokument>"
     ...
     *
     * The base XML must be serialized and re-parsed before signing so that sign and verify both
     * operate on a parsed DOM. A programmatically-built DOM (createElementNS) can produce a
     * different canonical form than a parsed DOM for the URI="" reference, causing the digest to
     * differ on round-trip even though the document bytes are identical.
     */
    private static byte[] sign(KeyPair kp, String ksefNumber) throws Exception {
        String baseXml = "<Potwierdzenie xmlns=\"" + NS + "\"><Dokument Id=\"SignedProperties\">"
                + "<NumerKSeFDokumentu>" + ksefNumber + "</NumerKSeFDokumentu></Dokument></Potwierdzenie>";
        ...
        Reference enveloped = fac.newReference("", sha256,
                List.of(fac.newTransform(Transform.ENVELOPED, (TransformParameterSpec) null)), null, null);
        Reference toProps = fac.newReference("#SignedProperties", sha256,
                List.of(fac.newTransform(CanonicalizationMethod.INCLUSIVE, (TransformParameterSpec) null)),
