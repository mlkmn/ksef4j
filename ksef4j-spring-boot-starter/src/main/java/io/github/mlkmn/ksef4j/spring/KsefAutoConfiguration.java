...
public class KsefAutoConfiguration {
  ...
  /**
   * Build the client from the bound properties. A user-supplied {@link InvoiceArchive} bean takes
   * precedence over {@code ksef.archive.directory}; if neither is set the client uses its no-op
   * archive. Backs off if the application already defines a {@link KsefClient}.
   *
  ...
  public KsefClient ksefClient(
      KsefProperties properties, ObjectProvider<InvoiceArchive> archiveProvider) {
    KsefClient.Builder builder =
        KsefClient.builder()
